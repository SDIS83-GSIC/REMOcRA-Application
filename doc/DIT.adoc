...
Dans ce dossier, ajouter un fichier nommé `compose.yaml`. Renseigner le contenu du fichier `compose-example.yaml`
...
REMOCRA_USERNAME_LENGTH_MIN=3
REMOCRA_USERNAME_LENGTH_MAX=255
...
CLIENT_SECRET_APACHE_HOP=xxxxxxxxxxxxxxxxx
...
| POSTGRES_DB_PORT | Port de la base de données PostgreSQL | 5432
...
| REMOCRA_USERNAME_LENGTH_MIN | Longueur minimale d'un username pour la création d'un compte dans l'application | 3
| REMOCRA_USERNAME_LENGTH_MAX | Longueur maximale d'un username pour la création d'un compte dans l'application | 255

.8+| *Informations de connexion à la base SIG du SDIS
(les valeurs sont optionnelles)*
| SIG_HOSTNAME | Nom de l'hôte de la base de données SIG |
| SIG_PORT_NUMBER | Port de la base de données SIG  |
| SIG_DATABASE_VENDOR | Fournisseur de la base de données SIG (`postgres` ou `oracle`)  |
| SIG_DBNAME | Nom de la base de données SIG |
| SIG_USERNAME | Nom de l’utilisateur de la base de données SIG |
| SIG_PASSWORD | Mot de passe de l'utilisateur de la base de données SIG |
| SIG_DRIVER_TYPE | Type de driver pour se connecter à la base de données SIG (uniquement pour oracle) |
| SIG_SERVICE_NAME | Service name de la base de données SIG (uniquement pour oracle) |
...
| CLIENT_SECRET_APACHE_HOP | Client secret du client apache hop (obligatoire) |
...
| NEXSIS_URL | Endpoint utilisé par nexsis pour créer, modifier ou supprimer un PEI | http://nexsis.gouv.fr/api/sig/point-eau-incendie
| NEXSIS_USER | Utilisateur de connexion à nexsis |
| NEXSIS_PASSWORD | Mot de passe de l'utilisateur |
| NEXSIS_TOKEN_ENDPOINT | Url de l'endpoint pour récupérer le token d'authentification auprès de nexsis |
...
# mkdir /var/opt/apache-hop # si Apache Hop est installé
...
docker compose run --rm \
    ...
    -e KEYCLOAK_USER=<super_username> \
    -e KEYCLOAK_PASSWORD=<super_password> \
    -e CLIENT_SECRET_APACHE_HOP=<client_secret_apache_hop> \
    -e REMOCRA_USERNAME_LENGTH_MIN=<remocra_username_length_min> \
    -e REMOCRA_USERNAME_LENGTH_MAX=<remocra_username_length_max> \
