...
REMOcRA est une application développée pour les SDIS, afin de leur permettre d’assurer toute la logique métier autour des PEI, ainsi que de faciliter le reporting dans certaines situations. REMOcRA utilise beaucoup de cartographie pour contextualiser les données.
...
Le déploiement de l’application requiert un serveur prenant en charge Docker. Il est fortement préconisé d’utiliser la dernière version LTS d’Ubuntu. La configuration recommandée est la suivante :
