...
class OldebRepository @Inject constructor(private val dsl: DSLContext) : AbstractRepository() {
    ...
    fun getList(params: Params<Filter, Sort>): List<OldebData> =
        dsl.with(lastOldebVisiteCte)
            ...
            .select(CADASTRE_PARCELLE.NUMERO.`as`("OLDEB_PARCELLE"))
            .select(CADASTRE_SECTION.NUMERO.`as`("OLDEB_SECTION"))
            .from(OLDEB)
    ...
    fun checkSectionAndParcelleIsUsed(oldebId: UUID?, communeId: UUID, sectionId: UUID, parcelleId: UUID?): Boolean =
        dsl.fetchExists(
            DSL.selectFrom(OLDEB)
                .where(
                    OLDEB.ID.notEqual(oldebId)
                        .and(OLDEB.COMMUNE_ID.eq(communeId))
                        .and(OLDEB.CADASTRE_SECTION_ID.eq(sectionId))
                        .and(OLDEB.CADASTRE_PARCELLE_ID.eq(parcelleId)),
                ),
        )
    ...
    fun getGeometrieOldeb(oldebId: UUID): OldebGeometrie =
        dsl.select(OLDEB.GEOMETRIE)
            .from(OLDEB)
            .where(OLDEB.ID.eq(oldebId))
            .fetchSingleInto()

    data class OldebGeometrie(
