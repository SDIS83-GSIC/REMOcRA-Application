...
class UpdateAdresseSousTypeElementUseCase @Inject constructor(private val adresseRepository: AdresseRepository) : AbstractCUDUseCase<AdresseSousTypeElement>(TypeOperation.UPDATE) {
    ...
    override fun execute(userInfo: UserInfo?, element: AdresseSousTypeElement): AdresseSousTypeElement {
        adresseRepository.update(element)
        return element
    }

    override fun checkContraintes(userInfo: UserInfo?, element: AdresseSousTypeElement) {
        if (adresseRepository.checkCodeExists(element.adresseSousTypeElementCode, element.adresseSousTypeElementId)) {
