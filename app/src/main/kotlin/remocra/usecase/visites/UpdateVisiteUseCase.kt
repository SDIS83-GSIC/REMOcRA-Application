...
class UpdateVisiteUseCase @Inject constructor(
    ...
    override fun checkContraintes(userInfo: WrappedUserInfo, element: VisiteData) {
        ...
        val last = visiteRepository.getLastVisite(element.visitePeiId)

        // On vérifie que c'est bien la dernière visite du PEI
        if (last?.visiteId != element.visiteId) {
            throw RemocraResponseException(ErrorType.VISITE_UPDATE_NOT_LAST)
        }
    }

    override fun execute(userInfo: WrappedUserInfo, element: VisiteData): VisiteData {
        visiteRepository.updateVisite(
            visiteId = element.visiteId,
            agent1 = element.visiteAgent1,
            agent2 = element.visiteAgent2,
            observation = element.visiteObservation,
        )
        ...
        anomalieRepository.deleteLVisiteAnomalie(element.visiteId)
