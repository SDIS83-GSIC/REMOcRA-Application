...
class CreateSignalementSousTypeElementUseCase @Inject constructor(private val signalementRepository: SignalementRepository) : AbstractCUDUseCase<SignalementSousTypeElement>(TypeOperation.INSERT) {
    ...
    override fun execute(userInfo: WrappedUserInfo, element: SignalementSousTypeElement): SignalementSousTypeElement {
        signalementRepository.insert(element)
        return element
    }

    override fun checkContraintes(userInfo: WrappedUserInfo, element: SignalementSousTypeElement) {
        if (signalementRepository.checkCodeExists(element.signalementSousTypeElementCode, null)) {
