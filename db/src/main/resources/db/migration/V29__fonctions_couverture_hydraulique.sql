CREATE OR REPLACE FUNCTION couverturehydraulique.is_gros_debit(peiid UUID, codesdis TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
AS $function$
DECLARE
    gros_deb BOOLEAN := FALSE;
    diametreNominal VARCHAR;
    naturePIBI VARCHAR;
    jumele_hydrant INT8;
    peiRecord RECORD;
    peiProjetRecord RECORD;
BEGIN
    CASE codesdis
        WHEN 'BSPP' THEN
            -- Récupération des informations du PEI
            SELECT * INTO peiRecord FROM remocra.pei WHERE pei_id = peiid;

            IF peiRecord IS NOT NULL THEN
                -- Récupérer le diamètre nominal et la nature
                SELECT d.diametre_code INTO diametreNominal
                FROM remocra.diametre d
                JOIN remocra.pibi p ON p.pibi_diametre_id = d.diametre_id
                WHERE p.pibi_id = peiid;

                SELECT n.nature_code INTO naturePIBI
                FROM remocra.nature n
                JOIN remocra.pei p ON p.pei_nature_id = n.nature_id
                WHERE p.pei_id = peiid;

                -- Vérifier si le pibi est jumelé
                SELECT pibi_jumele_id INTO jumele_hydrant FROM remocra.pibi WHERE pibi_id = peiid;

                -- Conditions pour déterminer le gros débit
                IF (naturePIBI = 'PI' AND diametreNominal = 'DIAM150') THEN
                    gros_deb := TRUE;
                ELSIF (naturePIBI = 'BI' AND diametreNominal = 'DIAM100' AND jumele_hydrant IS NOT NULL) THEN
                    gros_deb := TRUE;
                END IF;
            ELSE
                -- PEI en projet
                SELECT * INTO peiProjetRecord FROM couverturehydraulique.pei_projet WHERE pei_projet_id = peiid;

                IF peiProjetRecord IS NOT NULL THEN
                    SELECT diametre_code INTO diametreNominal
                    FROM remocra.diametre
                    WHERE diametre_id = peiProjetRecord.pei_projet_diametre_id;

                    IF (peiProjetRecord.pei_projet_diametre_canalisation >= 150 AND diametreNominal = 'DIAM150') THEN
                        gros_deb := TRUE;
                    END IF;
                END IF;
            END IF;
...
BEGIN
  FOR voie IN (SELECT * FROM couverturehydraulique.reseau WHERE reseau_etude_id IS NOT DISTINCT FROM idEtude) LOOP
    ...
    ELSE
      UPDATE couverturehydraulique.reseau SET reseau_sommet_destination = t_sommet_id WHERE reseau_id = voie.reseau_id;
    END IF;
  END LOOP;
END
$function$
        ...
        IF t IS NULL AND distanceParcourue <= dist THEN
          ...
          INSERT INTO couverturehydraulique.temp_distance (
            ...
            SELECT gen_random_uuid(), depart, voisinRecord.destination, voisinRecord.id, distanceParcourue, buffer,
                courantRecord.voie, voisinRecord.traversable, upper(bufferSide)::couverturehydraulique.type_side;
        END IF;
