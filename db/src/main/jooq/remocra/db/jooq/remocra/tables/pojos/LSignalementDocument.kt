/*
 * This file is generated by jOOQ.
 */
package remocra.db.jooq.remocra.tables.pojos

import java.io.Serializable
import java.util.UUID
import javax.annotation.processing.Generated

/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = [
        "https://www.jooq.org",
        "jOOQ version:3.20.10",
    ],
    comments = "This class is generated by jOOQ",
)
@Suppress("warnings")
data class LSignalementDocument(
    val documentId: UUID,
    val signalementId: UUID,
) : Serializable {

    override fun equals(other: Any?): Boolean {
        if (this === other) {
            return true
        }
        if (other == null) {
            return false
        }
        if (this::class != other::class) {
            return false
        }
        val o: LSignalementDocument = other as LSignalementDocument
        if (this.documentId != o.documentId) {
            return false
        }
        if (this.signalementId != o.signalementId) {
            return false
        }
        return true
    }

    override fun hashCode(): Int {
        val prime = 31
        var result = 1
        result = prime * result + this.documentId.hashCode()
        result = prime * result + this.signalementId.hashCode()
        return result
    }

    override fun toString(): String {
        val sb = StringBuilder("LSignalementDocument (")

        sb.append(documentId)
        sb.append(", ").append(signalementId)

        sb.append(")")
        return sb.toString()
    }
}
