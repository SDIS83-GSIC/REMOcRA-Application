...
`:db:flywayMigrateStandalone`:: permet de jouer les migrations Flyway sur une autre base de données autonome qui est réinitialisée à chaque fois.

== Régénérer le mapping des classes par rapport à la base de données

`:db:jooq`:: Ceci lancera également la migration Flyway (tâche `:db:flywayMigrate`)
...
`:db:jooqStandalone`:: Génère le code Kotlin jOOQ à partir du schéma de la base de données, sur une autre base de données autonome qui est réinitialisée à chaque fois. Cela simplifie le développement, car on génère le code jOOQ à partir de cette base de données locale sans pour autant modifier la base de données de développement.
